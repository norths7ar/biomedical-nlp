...
MAX_TOKENS = 8192
BATCH_SIZE = 100
PROMPT_FILE_NAME = "rename_chemical_abbr.md"
INPUT_FILE_NAME = "llm2_validate_disease.jsonl"
OUTPUT_FILE_NAME = "llm3_chemical_abbr.jsonl"
...
def build_entity_cache(output_path: Path) -> set[str]:
    ...
    for item in iter_jsonl(output_path):
        if item.get("decode_error", False):
            continue
        if not "is_abbr" in item:
            continue
        records.add(item["relationid"])
        pmid = item["pmid"]
        mention = item["chemical_name"]
        key = (mention, pmid)
        ENTITY_CACHE[key] = {"is_abbr": item["is_abbr"], "full_name": item["full_name"], "full_name_source": item["full_name_source"], "confidence": item["confidence"]}
    return records
...
def main():
    ...
    reference_file = PIPELINE_DIR.parent / "drug_protein" / "data" / dataset / OUTPUT_FILE_NAME
    ...
    try:
        for article_info in article_data:
            if article_info["relationid"] in completed_relationids:
                ...
                continue
            article_info["llm_step"] = "check_chemical_abbr"
            ...
            if key in ENTITY_CACHE:
                ...
                prompt = prompt_template.format(CHEMICAL=mention, TITLE=article_info["title"], ABSTRACT=article_info["abstract"])
