...
MAX_TOKENS = 8192
BATCH_SIZE = 100
PROMPT_FILE_NAME = "validate_drug.md"
INPUT_FILE_NAME = "stage_chemical_abbr_resolved.jsonl"
OUTPUT_FILE_NAME = "llm4_validate_chemical.jsonl"
REFERENCE_FILE_NAME = "llm2_validate_chemical.jsonl"
...
def build_entity_cache(output_path: Path) -> set[str]:
    ...
    for item in iter_jsonl(output_path):
        ...
        try:
            pmid = item["pmid"]
            entity_id = item["chemical_biokdeid"]
            mention = item["chemical_name"]
            valid = item.get("drug_valid", False)
...
def main():
    ...
    reference_file = PIPELINE_DIR.parent / "drug_protein" / "data" / dataset / REFERENCE_FILE_NAME
    ...
    try:
        for article_info in article_data:
            if article_info["relationid"] in completed_relationids:
                ...
                continue
            article_info["llm_step"] = "validate_drug"
            pmid = article_info["pmid"]
            entity_id = article_info["chemical_biokdeid"]
            mention = article_info["chemical_name"]
            if not mention:
            ...
            if cache_key in ENTITY_CACHE:
                ...
                article_info["drug_valid"] = ENTITY_CACHE[cache_key]
                article_info["decode_error"] = False
                completed_relationids.add(article_info["relationid"])
            else:
                prompt = prompt_template.format(DRUG=mention, TITLE=article_info["title"], ABSTRACT=article_info["abstract"])
                ...
                if response_dict:
                    ...
                    article_info["decode_error"] = False
                    ENTITY_CACHE[cache_key] = article_info["drug_valid"]
                    completed_relationids.add(article_info["relationid"])
                    new_processed_counter += 1
                else:
                    article_info["drug_valid"] = False
