...
PROMPT_FILE_NAME = "novelty.md"
INPUT_FILE_NAME = "stage_combination_therapy_applied.jsonl"
OUTPUT_FILE_NAME = "llm6_novelty.jsonl"
...
def main():
    ...
    try:
        for article_info in article_data:
            ...
            if article_info.get("drug_valid", False) is False:
                LOGGER.debug(
                    "Skipping %s because drug is not valid",
                    article_info["relationid"],
                )
                log_counter += 1
                continue

            article_info["llm_step"] = "novelty"
